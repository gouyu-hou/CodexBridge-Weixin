...
async function main(argv: string[] = process.argv.slice(2)) {
  ...
  if (group === 'weixin' && command === 'share') {
    return runWeixinShare(args);
  }
...
async function runWeixinLogin(args: string[]) {
  return runWeixinPairing(args, { defaultSaveFile: true, shareMode: false });
}

async function runWeixinShare(args: string[]) {
  return runWeixinPairing(args, { defaultSaveFile: false, shareMode: true });
}

async function runWeixinPairing(args: string[], {
  defaultSaveFile,
  shareMode,
}: {
  defaultSaveFile: boolean;
  shareMode: boolean;
}) {
  ...
  const credentials = await officialQrLogin({
    ...
    onQrCode: async ({ qrcode, qrcodeImageContent }) => {
      const output = await resolveQrArtifact({
        saveFile,
        stateDir,
        qrcode,
        qrcodeImageContent,
      ...
      process.stdout.write(`${i18n.t(shareMode ? 'cli.share.qrGenerated' : 'cli.login.qrGenerated')}\n`);
      ...
      process.stdout.write(`${i18n.t(shareMode ? 'cli.share.scanPrompt' : 'cli.login.scanPrompt')}\n`);
    },
  ...
  process.stdout.write(`${i18n.t(shareMode ? 'cli.share.success' : 'cli.login.success')}\n`);
  ...
  if (shareMode) {
    process.stdout.write(`${i18n.t('cli.share.restartHint')}\n`);
  }
}
...
async function runWeixinServe(args: string[]) {
  ...
  const runtime = createCodexBridgeRuntime({
    platformPlugins: [
      new MultiAccountWeixinPlatformPlugin({
        accountStore,
        stateDir,
        env: process.env,
        locale: i18n.locale,
        attachmentProcessingConcurrency: parsePositiveIntEnv(process.env.WEIXIN_ATTACHMENT_CONCURRENCY, 3),
        accountPollConcurrency: parsePositiveIntEnv(process.env.WEIXIN_ACCOUNT_POLL_CONCURRENCY, 4),
      }),
    ],
    providerPlugins: [
    ...
    restartBridge: async ({ event }) => {
      const externalScopeId = event?.externalScopeId ?? null;
      if (bridgeControlRef) {
        setTimeout(() => {
          void bridgeControlRef?.restart().catch(async (error) => {
            process.stderr.write(`[weixin] runtime restart failed: ${formatError(error)}\n`);
            await queueWeixinBridgeRestart({
              stateDir,
              externalScopeId,
            }).catch((restartError) => {
              process.stderr.write(`[weixin] external restart fallback failed: ${formatError(restartError)}\n`);
            });
          });
        }, 0);
        return;
      }
      await queueWeixinBridgeRestart({ stateDir, externalScopeId });
    },
  });
  const platformPlugin = runtime.registry.getPlatform('weixin') as MultiAccountWeixinPlatformPlugin;
  const bridgeRuntime = new WeixinBridgeRuntime({
    ...
    turnTimeoutMs: parsePositiveIntEnv(process.env.WEIXIN_TURN_TIMEOUT_MS, 0),
    pollTimeoutMs: parsePositiveIntEnv(process.env.WEIXIN_POLL_TIMEOUT_MS, 0),
    deliverProgressPreviews: parseBooleanEnv(process.env.WEIXIN_PROGRESS_PREVIEWS, false),
    maxConcurrentTurns: parsePositiveIntEnv(
      process.env.WEIXIN_MAX_CONCURRENT_TURNS,
      WeixinBridgeRuntime.DEFAULT_MAX_CONCURRENT_TURNS,
    ),
    eventDispatchConcurrency: parsePositiveIntEnv(
      process.env.WEIXIN_EVENT_DISPATCH_CONCURRENCY,
  ...
  const bridgeControl = {
    async start() {
      if (bridgeRuntime.isRunning() || bridgeStarting || bridgeStopping || bridgeRestarting) {
        return;
      }
      bridgeStarting = true;
      try {
        await bridgeRuntime.startInBackground();
      } finally {
        bridgeStarting = false;
      }
    },
    async stop() {
      if (!bridgeRuntime.isRunning() || bridgeStopping || bridgeRestarting) {
        return;
      }
      bridgeStopping = true;
      try {
    ...
    async restart() {
      if (bridgeStarting || bridgeStopping || bridgeRestarting) {
        return;
      }
      bridgeRestarting = true;
      try {
        await bridgeRuntime.restart({ force: true });
      } finally {
    ...
    async configureConcurrency(settings: {
      ...
      bridgeRuntime.configureConcurrency({
        maxConcurrentTurns: settings.maxConcurrentTurns,
        eventDispatchConcurrency: settings.eventDispatchConcurrency,
      });
      platformPlugin.configureConcurrency?.({
        attachmentProcessingConcurrency: settings.attachmentProcessingConcurrency,
        accountPollConcurrency: settings.accountPollConcurrency,
      });
    },
    status() {
      const runtimeStatus = bridgeRuntime.getStatus();
      return {
        ...runtimeStatus,
        running: bridgeRuntime.isRunning(),
        starting: bridgeStarting,
        stopping: bridgeStopping,
        restarting: bridgeRestarting,
        weixin: platformPlugin.getStatus?.() ?? null,
      };
    },
    getMetrics() {
  ...
  const adminServer = adminOptions.enabled
    ? new WeixinAdminServer({
      ...
      stateDir,
      env: process.env,
      host: adminOptions.host,
      port: adminOptions.port,
      locale: i18n.locale,
      bridgeControl,
      serviceControl: {
        shutdown: (reason = 'admin-service-shutdown') => {
          setTimeout(() => {
            void stop(reason);
          }, 0);
        },
  ...
  process.stdout.write(`weixin_admin_enabled: ${adminServer ? 'true' : 'false'}\n`);
  ...
  try {
    ...
    if (adminServer) {
      try {
        const binding = await adminServer.start();
        process.stdout.write(`weixin_admin_url: ${binding.url}\n`);
      } catch (error) {
        process.stderr.write(`[weixin-admin] ${formatError(error)}\n`);
      }
    }
...
async function resolveQrArtifact({ saveFile, stateDir, qrcode, qrcodeImageContent }: {
  saveFile: boolean;
  stateDir: string;
  qrcode: string;
  qrcodeImageContent: string | null | undefined;
  ...
  if (saveFile) {
    return materializeQrArtifact({ stateDir, qrcode, qrcodeImageContent });
  }
  if (typeof qrcodeImageContent === 'string' && /^https?:\/\//u.test(qrcodeImageContent)) {
    return { filePath: null, sourceUrl: qrcodeImageContent };
  }
...
async function acquireServeLock(lockPath: string): Promise<ServeLock> {
  ...
  if (isServeLockOwnerAlive(existing)) {
...
function isServeLockOwnerAlive(lock: ServeLockPayload | null): lock is ServeLockPayload {
  const pid = Number(lock?.pid ?? 0);
  if (!isProcessAlive(pid)) {
    return false;
  }
  if (pid === process.pid || process.platform !== 'win32') {
    return true;
  }
  const commandLine = readWindowsProcessCommandLine(pid);
  return commandLine === null ? true : isServeLockOwnerCommandLine(commandLine);
}

function readWindowsProcessCommandLine(pid: number): string | null {
  try {
...
function isServeLockOwnerCommandLine(commandLine: string): boolean {
  const normalized = String(commandLine ?? '').replace(/\\/gu, '/').toLowerCase();
  if (!normalized.includes('/codexbridge/')) {
    return false;
  }
  return (
    normalized.includes('/src/cli.ts weixin serve')
    || normalized.includes('/src/cli.ts codex native-api-serve')
    || normalized.includes('/dist/src/cli.js weixin serve')
    || normalized.includes('/dist/src/cli.js codex native-api-serve')
  );
}
