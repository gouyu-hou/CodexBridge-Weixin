...
} from '../../../packages/codex-native-api/src/codex_app_turn_policy.js';
import {
  findCodexSessionIndexEntry,
  mergeCodexSessionIndexThreads,
  readCodexSessionIndex,
  resolveCodexHome,
} from './session_index.js';
import type {
  ProviderAppInfo,
...
export class CodexAppClient extends EventEmitter {
  ...
  constructor({
    ...
    codexHome = process.env.CODEX_HOME ? resolveCodexHome(process.env) : null,
  ...
  isTransportConnected(): boolean {
    ...
    return Boolean(this.socket && readWebSocketReadyState(this.socket) === readWebSocketOpenState());
  }
  ...
  async listThreads({
    ...
    const sessionIndex = this.codexHome
      ? readCodexSessionIndex({ codexHome: this.codexHome })
      : [];
    const mappedRows = rows.map(mapThreadSummary);
    return {
      items: mergeCodexSessionIndexThreads({
        items: mappedRows,
        sessionIndex,
        searchTerm,
        archived,
  ...
  async readThread(threadId: string, includeTurns = false): Promise<ProviderThreadSummary | null> {
    let result: any = null;
    try {
      result = await this.request('thread/read', { threadId, includeTurns }, { timeoutMs: 10_000 });
    } catch (error) {
      const indexEntry = this.codexHome
        ? findCodexSessionIndexEntry(threadId, { codexHome: this.codexHome, requireSessionFile: true })
        : null;
      if (!isThreadReadNotFoundError(error)) {
        throw error;
      }
      if (!indexEntry) {
        return null;
    ...
    if (result?.thread) {
      const thread = mapThread(result.thread, includeTurns);
      if (thread.title) {
        return thread;
      }
      const indexEntry = this.codexHome
        ? findCodexSessionIndexEntry(thread.threadId, { codexHome: this.codexHome })
        : null;
      return indexEntry ? {
        ...thread,
        title: indexEntry.title ?? thread.title,
        cwd: thread.cwd ?? indexEntry.cwd,
        path: thread.path ?? indexEntry.path,
        updatedAt: thread.updatedAt ?? indexEntry.updatedAt,
      } : thread;
    }
    const indexEntry = this.codexHome
      ? findCodexSessionIndexEntry(threadId, { codexHome: this.codexHome, requireSessionFile: true })
  ...
  async listPlugins({
    ...
    let result: CodexAppPluginListResponse;
    try {
      result = await this.request('plugin/list', {
        cwds: cwd ? [cwd] : [],
      }, { timeoutMs: 30_000 });
    } catch (error) {
      if (isUnsupportedAppServerRpcError(error, 'plugin/list')) {
        this.logDebug('app_server_rpc_unsupported', {
          method: 'plugin/list',
          fallback: 'empty_plugin_catalog',
        });
  ...
  async readPlugin({
    ...
    let result: any;
    try {
      result = await this.request('plugin/read', params, { timeoutMs: 30_000 });
    } catch (error) {
      if (isUnsupportedAppServerRpcError(error, 'plugin/read')) {
        this.logDebug('app_server_rpc_unsupported', {
          method: 'plugin/read',
          fallback: 'null_plugin_detail',
        });
        return null;
      }
      throw error;
  ...
  async listApps(): Promise<ProviderAppInfo[]> {
    ...
    do {
      ...
      try {
        ...
        if (isUnsupportedAppServerRpcError(error, 'app/list')) {
          this.logDebug('app_server_rpc_unsupported', {
            method: 'app/list',
            fallback: 'empty_app_list',
          });
          return [];
        }
        throw error;
  ...
  async startServerWithTransport(transportKind: 'websocket' | 'stdio'): Promise<void> {
    ...
    const appServerArgVariants = transportKind === 'websocket'
    ...
    let lastError: Error | null = null;
    for (let index = 0; index < appServerArgVariants.length; index += 1) {
      const appServerArgs = appServerArgVariants[index]!;
      try {
        await this.startServerWithTransportArgs(transportKind, appServerArgs);
        return;
      } catch (error) {
        lastError = error instanceof Error ? error : new Error(String(error ?? 'Unknown Codex app-server start error'));
        const willRetry = index < appServerArgVariants.length - 1;
        this.logDebug('app_server_transport_variant_failed', {
          transportKind,
          willRetry,
          spawnArgs: appServerArgs,
          error: lastError.message,
        });
        await this.stop();
        if (!willRetry) {
  ...
  async startServerWithTransportArgs(transportKind: 'websocket' | 'stdio', appServerArgs: string[]): Promise<void> {
    this.childStartError = null;
    this.childStderrTail = [];
    this.stdioLineBuffer = '';
    ...
    this.child.on('exit', () => {
      ...
      const exitError = createCodexAppServerExitedError({
        command: launchSpec.displayCommand,
        exitCode: spawnedChild.exitCode ?? -1,
  ...
  async connectWebSocket(): Promise<void> {
    ...
    while (Date.now() - started < APP_SERVER_CONNECT_TIMEOUT_MS) {
      ...
      try {
        ...
        if (isMissingWebSocketRuntimeError(error)) {
          throw error;
        }
        await sleep(250);
      }
  ...
  async initialize(): Promise<void> {
    ...
    }, { timeoutMs: 30_000, autoStart: false });
  ...
  async request(
    method: string,
    params: any,
    { timeoutMs = 30_000, autoStart = true }: { timeoutMs?: number; autoStart?: boolean } = {},
  ): Promise<any> {
    if (!this.isTransportConnected()) {
      if (!autoStart) {
        throw new Error(`Codex app-server is not connected while sending ${method}`);
      }
      await this.start();
    }
    ...
    return new Promise((resolve, reject) => {
      ...
      try {
        ...
        const sendError = error instanceof Error ? error : new Error(String(error ?? 'Unknown Codex JSON-RPC send error'));
  ...
  send(payload: any): void {
    ...
    if (!this.socket || readWebSocketReadyState(this.socket) !== readWebSocketOpenState()) {
...
function emptyPluginCatalog(): ProviderPluginsListResult {
  return {
    featuredPluginIds: [],
...
function isUnsupportedAppServerRpcError(error: unknown, method: string): boolean {
  const message = error instanceof Error ? error.message : String(error ?? '');
  return message.includes('unknown variant')
    && (message.includes(`\`${method}\``) || message.includes(`"${method}"`) || message.includes(method));
}

function isThreadReadNotFoundError(error: unknown): boolean {
  const message = error instanceof Error ? error.message : String(error ?? '');
  return /thread\/read/i.test(message) && /(not found|unknown|missing|no such)/i.test(message)
    || /\bthread\b.*\b(not found|unknown|missing|no such)\b/i.test(message);
}
...
function createDefaultWebSocket(url: string): WebSocket {
  ...
  if (!WebSocketCtor) {
    throw new Error('WebSocket is not available in this Node.js runtime. Use Node.js 22+ or set CODEX_APP_SERVER_TRANSPORT=stdio.');
  }
  return new WebSocketCtor(url);
}

function readWebSocketOpenState(): number {
  const WebSocketCtor = (globalThis as { WebSocket?: { OPEN?: number } }).WebSocket;
  return typeof WebSocketCtor?.OPEN === 'number' ? WebSocketCtor.OPEN : 1;
}

function readWebSocketReadyState(socket: WebSocket | null): number | null {
  const readyState = (socket as { readyState?: unknown } | null)?.readyState;
  return typeof readyState === 'number' ? readyState : null;
}

function isMissingWebSocketRuntimeError(error: unknown): boolean {
  const message = error instanceof Error ? error.message : String(error ?? '');
  return /WebSocket is not available/i.test(message);
}
