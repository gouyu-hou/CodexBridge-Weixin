...
export class BridgeSessionService {
  ...
  async listProviderThreads(
    ...
    const appendRemoteThreads = async (remoteArchived: boolean) => {
      ...
      while (true) {
        ...
        for (const remoteThread of remoteThreads) {
          ...
          const alias = normalizeThreadDisplayName(metadata?.alias);
          ...
          const localSettings = localSession ? this.sessionSettings.get(localSession.id) : null;
          const lastUserPrompt = resolveLastUserPrompt(localSettings?.metadata);
          const updatedAt = resolveLatestThreadUpdatedAt(
            remoteThread.updatedAt,
            localSession?.updatedAt,
            alias ? metadata?.updatedAt : null,
          );
          remoteItems.push({
            ...
            updatedAt,
            preview: lastUserPrompt ?? remoteThread.preview ?? null,
    ...
    for (const localSession of localSessions) {
      ...
      const localSettings = this.sessionSettings.get(localSession.id);
      const lastUserPrompt = resolveLastUserPrompt(localSettings?.metadata);
      ...
      remoteItems.push({
        ...
        updatedAt: resolveLatestThreadUpdatedAt(localSession.updatedAt, alias ? metadata?.updatedAt : null),
        preview: lastUserPrompt,
        turns: [],
    ...
    for (const metadata of metadataByThreadId.values()) {
      if (!metadata.threadId || seenThreadIds.has(metadata.threadId)) {
        continue;
      }
      const alias = normalizeThreadDisplayName(metadata.alias);
      if (!alias) {
      ...
      seenThreadIds.add(metadata.threadId);
      remoteItems.push({
    ...
    const filteredItems = remoteItems
      ...
      .filter((item) => threadMatchesSearchTerm(item, searchTerm))
      .sort((left, right) => {
  ...
  findProviderThreadsByDisplayName(providerProfileId: string, name: string) {
    const nameKey = normalizeThreadDisplayNameKey(name);
    if (!nameKey) {
      return [];
    }
    const metadataByThreadId = new Map(
    ...
    const addMatch = ({
      ...
      matches.set(normalizedThreadId, {
    ...
    for (const metadata of metadataByThreadId.values()) {
      const alias = normalizeThreadDisplayName(metadata.alias);
      if (alias && normalizeThreadDisplayNameKey(alias) === nameKey) {
        addMatch({
          threadId: metadata.threadId,
          alias,
          title: alias,
    ...
    for (const session of this.bridgeSessions.listByProviderProfileId(providerProfileId)) {
      const metadata = metadataByThreadId.get(session.codexThreadId) ?? null;
      const alias = normalizeThreadDisplayName(metadata?.alias);
      const displayTitle = normalizeThreadDisplayName(alias ?? session.title);
      if (displayTitle && normalizeThreadDisplayNameKey(displayTitle) === nameKey) {
        addMatch({
          threadId: session.codexThreadId,
          alias,
          title: displayTitle,
          updatedAt: typeof metadata?.updatedAt === 'number' ? metadata.updatedAt : session.updatedAt,
          archivedAt: typeof metadata?.archivedAt === 'number' ? metadata.archivedAt : null,
          pinnedAt: typeof metadata?.pinnedAt === 'number' ? metadata.pinnedAt : null,
        });
      }
    ...
    return [...matches.values()].sort((left, right) => Number(right.updatedAt ?? 0) - Number(left.updatedAt ?? 0));
  }
  ...
  async readProviderThread(
    ...
    return {
      ...
      preview: lastUserPrompt ?? thread.preview ?? null,
  ...
  renameProviderThread(providerProfileId: string, threadId: string, alias: string) {
    const normalizedAlias = normalizeThreadDisplayName(alias) ?? '';
...
function normalizeThreadDisplayName(value: unknown): string | null {
  const normalized = String(value ?? '').replace(/\s+/gu, ' ').trim();
  return normalized || null;
}

function normalizeThreadDisplayNameKey(value: unknown): string | null {
  return normalizeThreadDisplayName(value)?.toLocaleLowerCase() ?? null;
}

function threadMatchesSearchTerm(
  item: { threadId?: string | null; title?: string | null; preview?: string | null },
  searchTerm: string | null | undefined,
): boolean {
  const searchKey = normalizeThreadDisplayNameKey(searchTerm);
  if (!searchKey) {
    return true;
  }
  const haystack = normalizeThreadDisplayNameKey([
    item.threadId,
    item.title,
    item.preview,
  ].filter(Boolean).join(' '));
  return Boolean(haystack?.includes(searchKey));
}

function resolveLastUserPrompt(metadata: Record<string, unknown> | null | undefined): string | null {
  const lastUserPrompt = readTextFromMetadataValue(metadata?.lastUserPrompt);
  if (lastUserPrompt) {
    return lastUserPrompt;
  }
  const lastRetryableRequest = metadata?.lastRetryableRequest;
  if (lastRetryableRequest && typeof lastRetryableRequest === 'object') {
    return normalizeThreadDisplayName((lastRetryableRequest as { text?: unknown }).text);
  }
  return null;
}

function readTextFromMetadataValue(value: unknown): string | null {
  if (typeof value === 'string') {
    return normalizeThreadDisplayName(value);
  }
  if (value && typeof value === 'object') {
    return normalizeThreadDisplayName((value as { text?: unknown }).text);
  }
  return null;
}

function resolveLatestThreadUpdatedAt(...values: unknown[]): number | null {
