...
import {
  ...
  extractAutomationAddBody,
  extractAutomationEditBody,
  extractAutomationNaturalBody,
  extractAutomationRenameTitle,
  formatAutomationIntervalLabel,
  parseAutomationAddSpec,
  resolveAutomationCommand,
} from './automation_command.js';
import {
...
export class BridgeCoordinator {
  ...
  getPendingNewSession(scopeRef): PendingNewSessionRequest | null {
    return this.pendingNewSessionsByScope.get(formatPlatformScopeKey(scopeRef.platform, scopeRef.externalScopeId)) ?? null;
  }

  getPendingNewSessionProviderProfile(scopeRef): any | null {
    const pending = this.getPendingNewSession(scopeRef);
    if (!pending) {
      return null;
    }
    return this.requireProviderProfile(pending.providerProfileId);
  }

  consumePendingNewSession(scopeRef): PendingNewSessionRequest | null {
    const key = formatPlatformScopeKey(scopeRef.platform, scopeRef.externalScopeId);
    const pending = this.pendingNewSessionsByScope.get(key) ?? null;
    if (pending) {
      this.pendingNewSessionsByScope.delete(key);
    }
    return pending;
  }

  clearPendingNewSession(scopeRef): void {
    this.pendingNewSessionsByScope.delete(formatPlatformScopeKey(scopeRef.platform, scopeRef.externalScopeId));
  }
  ...
  updatePendingNewSessionSettings(
    ...
    const next: PendingNewSessionRequest = {
      ...pending,
      settings: {
        ...pending.settings,
        ...updates,
        metadata: {
          ...(pending.settings.metadata ?? {}),
          ...(updates.metadata ?? {}),
        },
      },
    };
    this.pendingNewSessionsByScope.set(key, next);
    return next;
  }

  updatePendingNewSessionProviderProfile(scopeRef, providerProfileId: string): PendingNewSessionRequest | null {
    const key = formatPlatformScopeKey(scopeRef.platform, scopeRef.externalScopeId);
    const pending = this.pendingNewSessionsByScope.get(key) ?? null;
    if (!pending) {
      return null;
  ...
  async handleConversationTurn(event, options = {}) {
    ...
    if (isUnreadableExternalShareText(effectiveEvent)) {
      debugCoordinator('conversation_turn_blocked_unreadable_external_share', {
        platform: scopeRef.platform,
        scopeId: scopeRef.externalScopeId,
        textPreview: truncateCoordinatorText(effectiveEvent?.text, 160),
      });
      return messageResponse([
        '这类外部 App 分享口令我无法直接读取原文内容。请把文章正文粘贴过来，或发文章截图/图片后再让我分析。',
      ], currentSession ? buildSessionMeta(currentSession) : this.buildScopedSessionMeta(effectiveEvent));
    }
      ...
      if (currentSession) {
        ...
        if (pendingNewSession) {
          session = await this.bridgeSessions.createSessionForScope(scopeRef, {
            providerProfileId: pendingNewSession.providerProfileId,
            cwd: pendingNewSession.cwd ?? currentSession.cwd ?? this.resolveEventCwd(effectiveEvent),
            initialSettings: {
              ...pendingNewSession.settings,
              locale,
            },
            providerStartOptions: {
              sourcePlatform: effectiveEvent.platform,
              trigger: 'new-command-first-message',
              previousBridgeSessionId: pendingNewSession.previousBridgeSessionId,
              previousThreadId: pendingNewSession.previousThreadId,
            },
          });
        } else {
  ...
  async handleStatusCommand(event, args = []) {
    ...
    const settings = session
      ...
      : pendingNewSession?.settings ?? null;
    ...
    if (!session) {
      const lines = [
        ...
        ...(pendingNewSession ? [this.t('coordinator.new.statusPending')] : []),
  ...
  async handleNewCommand(event, args) {
    ...
    const cwd = args.join(' ').trim() || existing?.cwd || this.resolveEventCwd(event);
    this.pendingNewSessionsByScope.set(scopeKey, {
      providerProfileId,
      cwd: normalizeCwd(cwd) ?? null,
      settings: this.buildCleanReboundSessionSettings(existingSettings, {
        locale: this.resolveScopeLocale(scopeRef, event),
      },
      ),
      previousBridgeSessionId: existing?.id ?? null,
      previousThreadId: existing?.codexThreadId ?? null,
      createdAt: this.now(),
    });
    return messageResponse([
      this.t('coordinator.new.pending'),
  ...
  async handleUploadsConversationTurn(event, scopeRef, session, uploadState, options: StartTurnOptions = {}) {
    ...
    if (!submissionText) {
      ...
      for (const [index, item] of newItems.entries()) {
        lines.push(this.renderUploadItemPreview(item, uploadState.items.length + index + 1));
      }
  ...
  renderUploadItemPreview(item: UploadBatchItem, index: number) {
    const details = [
      item.mimeType,
      typeof item.sizeBytes === 'number' ? formatBinarySize(item.sizeBytes) : '',
      typeof item.durationSeconds === 'number' ? `${item.durationSeconds}s` : '',
    ].filter(Boolean).join(', ');
    const transcript = item.transcriptText ? ` | ${truncateInlineText(item.transcriptText, 80)}` : '';
    const suffix = details ? ` (${details})` : '';
    return this.t('coordinator.uploads.item', {
      index,
      kind: this.t(`coordinator.uploads.kind.${item.kind}`),
      name: `${item.fileName ?? path.basename(item.localPath)}${suffix}${transcript}`,
    });
  }
  ...
  async handleOpenCommand(event, args) {
    ...
    const resolvedThread = await this.resolveRequestedThread(event, requested);
  ...
  async handlePlanCommand(event, args) {
    ...
    if (!normalizedArgs.length) {
      const pendingMode = pendingNewSession?.settings?.collaborationMode ?? null;
  ...
  async handleRenameCommand(event, args) {
    ...
    const resolvedThread = await this.resolveRequestedThread(event, target);
    if (!resolvedThread.ok) {
      return messageResponse([resolvedThread.message], this.buildScopedSessionMeta(event));
    }
    const normalizedNextName = compactWhitespace(nextName);
    const duplicateThread = this.bridgeSessions
      .findProviderThreadsByDisplayName(resolvedThread.providerProfileId, normalizedNextName)
      .find((thread) => thread.threadId !== resolvedThread.threadId) ?? null;
    if (duplicateThread) {
      return messageResponse([
        this.t('coordinator.rename.duplicateName', {
          name: normalizedNextName,
          threadId: duplicateThread.threadId,
        }),
      ], this.buildScopedSessionMeta(event));
    }
    this.bridgeSessions.renameProviderThread(resolvedThread.providerProfileId, resolvedThread.threadId, nextName);
    this.patchThreadBrowserTitle(event, resolvedThread.providerProfileId, resolvedThread.threadId, normalizedNextName);
    return textResponse([
      this.t('coordinator.rename.updated'),
      this.t('coordinator.rename.name', { name: normalizedNextName }),
  ...
  async handleProviderCommand(event, args) {
    ...
    if (args.length === 0) {
      ...
      return messageResponse([
        this.t('coordinator.provider.current', { id: current?.providerProfileId ?? pendingNewSession?.providerProfileId ?? this.resolveDefaultProviderProfileId() }),
        this.t('coordinator.provider.available'),
        ...profiles,
      ], current ? buildSessionMeta(current) : this.buildScopedSessionMeta(event));
    }
    const requested = args.join(' ').trim();
    ...
    const current = this.bridgeSessions.resolveScopeSession(scopeRef);
    if (pendingNewSession) {
      this.updatePendingNewSessionProviderProfile(scopeRef, profile.id);
      return messageResponse([
  ...
  async handlePermissionsCommand(event, args) {
    ...
    if (args.length === 0) {
      const settings = pendingNewSession?.settings ?? (session ? this.bridgeSessions.getSessionSettings(session.id) : null);
      return messageResponse(renderPermissionsLines(settings, this.currentI18n), session ? buildSessionMeta(session) : this.buildScopedSessionMeta(event));
    }
    ...
    const resolved = resolvePermissionsState({
      ...(pendingNewSession?.settings ?? (session ? this.bridgeSessions.getSessionSettings(session.id) : null)),
      ...update,
    });
    return messageResponse([
  ...
  resolveScopeProviderProfile(scopeRef) {
    const current = this.bridgeSessions.resolveScopeSession(scopeRef);
    const pending = this.getPendingNewSession(scopeRef);
    const providerProfileId = current?.providerProfileId
  ...
  async forceClearActiveTurn(scopeRef) {
    const activeTurn = this.activeTurns?.resolveScopeTurn(scopeRef) ?? null;
    if (!activeTurn) {
      return null;
    }
    this.activeTurns?.clearPendingApprovals(scopeRef);
    const cleared = this.activeTurns?.endScopeTurn(scopeRef) ?? activeTurn;
    debugCoordinator('active_turn_force_cleared', {
      platform: scopeRef.platform,
      scopeId: scopeRef.externalScopeId,
      bridgeSessionId: activeTurn.bridgeSessionId ?? null,
      threadId: activeTurn.threadId ?? null,
      turnId: activeTurn.turnId ?? null,
  ...
  async resolveRequestedThread(event, requested, options: { stateOverride?: any } = {}) {
    ...
    const current = this.bridgeSessions.resolveScopeSession(scopeRef);
    const providerProfileId = state?.providerProfileId ?? current?.providerProfileId ?? this.resolveDefaultProviderProfileId();
    if (isCurrentThreadTarget(value)) {
      if (!current) {
        return {
          ok: false,
          message: this.t('coordinator.thread.noCurrent'),
        };
      }
      const metadata = this.bridgeSessions.getThreadMetadata(current.providerProfileId, current.codexThreadId);
      return {
        ok: true,
    ...
    const exactPageNameMatches = state?.providerProfileId === providerProfileId
      ? state.items.filter((item) => normalizeThreadNameForLookup(item.title) === normalizeThreadNameForLookup(value))
      : [];
    if (exactPageNameMatches.length === 1) {
      const item = exactPageNameMatches[0];
      return {
        ok: true,
        providerProfileId,
        threadId: item.threadId,
    ...
    if (exactPageNameMatches.length > 1) {
      return {
        ok: false,
        message: this.t('coordinator.thread.ambiguousName', { name: value }),
      };
    }
    const exactNameMatches = this.bridgeSessions.findProviderThreadsByDisplayName(providerProfileId, value);
    if (exactNameMatches.length === 1) {
      const item = exactNameMatches[0];
      return {
        ok: true,
        providerProfileId,
        threadId: item.threadId,
    ...
    const indexedNameMatches = await this.findProviderThreadsByListedDisplayName(providerProfileId, value);
    if (indexedNameMatches.length === 1) {
      const item = indexedNameMatches[0];
      return {
        ok: true,
        providerProfileId,
        threadId: item.threadId,
  ...
  async resolveRequestedThreads(event, requestedValues) {
    const state = this.getThreadBrowserState(event);
    ...
    for (const requested of requestedValues) {
      resolved.push(await this.resolveRequestedThread(event, requested, { stateOverride: stateSnapshot }));
    }
    return resolved;
  }

  async findProviderThreadsByListedDisplayName(providerProfileId, name) {
    const normalizedName = normalizeThreadNameForLookup(name);
    if (!normalizedName) {
      return [];
    }
    try {
      ...
      return items
        .filter((item) => normalizeThreadNameForLookup(item?.title) === normalizedName)
  ...
  buildReboundSessionSettings(
    currentSettings: Partial<SessionSettings> | null,
  ...
  buildCleanReboundSessionSettings(
    currentSettings: Partial<SessionSettings> | null,
    overrides: Partial<SessionSettings> = {},
  ): Partial<SessionSettings> {
    return {
      ...
      collaborationMode: currentSettings?.collaborationMode ?? null,
      personality: currentSettings?.personality ?? null,
      permissionsMode: currentSettings?.permissionsMode ?? null,
      accessPreset: currentSettings?.accessPreset ?? null,
      approvalPolicy: currentSettings?.approvalPolicy ?? null,
      sandboxMode: currentSettings?.sandboxMode ?? null,
...
function buildLastUserPromptSnapshot(text: unknown, storedAt: number) {
  const normalized = compactWhitespace(text);
  return normalized ? { text: normalized, storedAt } : null;
}
...
function normalizeThreadNameForLookup(value: unknown): string {
  return compactWhitespace(value).toLocaleLowerCase();
}

function isCurrentThreadTarget(value: unknown): boolean {
  const normalized = normalizeThreadNameForLookup(value);
...
function getCommandHelpSpecs(i18n: Translator) {
  ...
  open: freezeCommandHelp({
    ...
    examples: [
      '/open 2',
      `/open ${i18n.t('common.example.aliasName')}`,
...
function buildPluginCategoryBuckets(details: ProviderPluginDetail[], i18n: Translator): PluginCategoryBucket[] {
  ...
  return Array.from(seed.values())
    ...
    .sort((left, right) => {
      ...
      const priority = pluginCategorySortPriority(left.key) - pluginCategorySortPriority(right.key);
      if (priority !== 0) {
        return priority;
      }
...
function pluginCategorySortPriority(key: string): number {
  switch (key) {
    case 'app':
      return 10;
    case 'mcp':
...
function isUnreadableExternalShareText(event: unknown): boolean {
  const text = String((event as { text?: unknown } | null)?.text ?? '').trim();
  if (!text) {
    return false;
  }
  const attachments = (event as { attachments?: unknown } | null)?.attachments;
  if (Array.isArray(attachments) && attachments.length > 0) {
    return false;
  }
  const normalized = text.replace(/\s+/gu, ' ');
  if (!isExternalAppSharePointer(normalized)) {
    return false;
  }
  return !hasReadableExternalShareContent(text);
}

function isExternalAppSharePointer(text: string): boolean {
  const hasShareOpenInstruction = /(长按复制打开|复制打开|打开抖音|打开小红书|打开快手|打开微博|打开今日头条|打开西瓜视频|打开B站|打开哔哩哔哩)/u
    .test(text);
  if (!hasShareOpenInstruction) {
    return false;
  }
  const hasExternalAppName = /(抖音|小红书|快手|微博|今日头条|西瓜视频|B站|哔哩哔哩)/u.test(text);
  const hasShareCodeLikeToken = /[A-Za-z0-9]{6,}|\^\^|[￥#].+[￥#]/u.test(text);
  return hasExternalAppName || hasShareCodeLikeToken;
}

function hasReadableExternalShareContent(text: string): boolean {
  const explicitContentMatch = text.match(/(?:文章正文|原文内容|正文内容|正文|原文|内容如下|全文)[:：]\s*([\s\S]+)/u);
  if (explicitContentMatch && countReadableCjkChars(explicitContentMatch[1] ?? '') >= 40) {
    return true;
  }
  const stripped = stripExternalShareBoilerplate(text);
  if (countReadableCjkChars(stripped) < 180) {
    return false;
  }
  const sentenceCount = (stripped.match(/[。！？!?；;]/gu) ?? []).length;
  const lineCount = stripped.split(/\r?\n/u).map((line) => line.trim()).filter(Boolean).length;
  return sentenceCount >= 3 || lineCount >= 4;
}

function stripExternalShareBoilerplate(text: string): string {
  return String(text ?? '')
    .replace(/【[^】]{1,120}】/gu, ' ')
    .replace(/[A-Za-z0-9]{1,3}:\/[^\n]{0,160}(?:\^\^|$)/gu, ' ')
    .replace(/[\s\S]{0,80}(?:长按复制打开|复制打开|打开抖音|打开小红书|打开快手|打开微博|打开今日头条|打开西瓜视频|打开B站|打开哔哩哔哩)[^\n。！？!?]{0,120}/gu, ' ')
    .replace(/[Š￥#][^\s\n]{4,80}(?:\^\^|[￥#])?/gu, ' ')
    .replace(/帮我(?:按照|根据)?(?:这个|这篇)?(?:文章|原文|内容)[^\n。！？!?]{0,120}/gu, ' ')
    .replace(/(?:我只要|只要|只需要|仅需要|即可)[^\n。！？!?]{0,120}/gu, ' ')
    .trim();
}

function countReadableCjkChars(text: string): number {
...
function isTemporaryProviderUnavailableError(error) {
  const message = error instanceof Error ? error.message : String(error);
  return /\bhttp\s+503\b/i.test(message)
    || /unexpected status 503\b/i.test(message)
...
function classifyTurnFailure(error, i18n: Translator) {
  ...
  if (/Timed out waiting for Codex turn/i.test(message) || isTemporaryProviderUnavailableError(message)) {
    return {
      outputState: 'timeout',
      errorMessage: readRecentCodexRuntimeError(undefined, i18n) || i18n.t('runtime.error.timeout'),
    };
  }
