...
function aggregateUnits(units: string[], maxLength: number) {
  const chunks: string[] = [];
  let current = '';
  const separator = shouldPreserveCommandListLineBreaks(units) ? '  \n' : '\n';
  for (const unit of units) {
    const normalizedUnit = String(unit ?? '').trim();
    if (!normalizedUnit) {
      continue;
    }
    const unitSeparator = current ? separator : '';
    const candidate = `${current}${unitSeparator}${normalizedUnit}`;
...
function shouldPreserveCommandListLineBreaks(units: string[]) {
  const normalized = units.map((unit) => String(unit ?? '').trim()).filter(Boolean);
  const hasCommandTitle = normalized.some((unit) => unit === '常用命令：' || unit === 'Common commands:');
  if (!hasCommandTitle) {
    return false;
  }
  return normalized.some((unit) => /^\/(?:new|stop|retry|reconnect|model|provider|up|rename|search|open|status|threads|next|prev|compact)\b/u.test(unit));
}
