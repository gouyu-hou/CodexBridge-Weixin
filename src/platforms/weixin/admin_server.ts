...
import {
  ...
  buildSetupRepairHint as buildDiagnosticsSetupRepairHint,
  buildSetupTestMessage as buildDiagnosticsSetupTestMessage,
  summarizeDiagnosticChecks as summarizeDiagnostics,
} from './admin_diagnostics_service.js';
import {
...
export class WeixinAdminServer {
  constructor({
    ...
    this.diagnosticsService = new WeixinAdminDiagnosticsService({
      ...
      env: this.env,
      adminPort: this.port,
      getBridgeStatus: () => this.bridgeControl?.status() ?? null,
      getAdminBinding: () => this.binding,
      resolveModelProviderSettings: () => this.resolveModelProviderSettings(),
    });
    this.server = null;
    this.binding = null;
  ...
  diagnosticsService: WeixinAdminDiagnosticsService;
  server: Server | null;
  ...
  private async handleRunDiagnostics(res: ServerResponse) {
    const checks = await this.diagnosticsService.runAll();
    this.writeJson(res, 200, {
      generatedAt: new Date().toISOString(),
  ...
  private async handleSetupTest(req: IncomingMessage, res: ServerResponse) {
    ...
    if (!['api-key', 'weixin', 'codex-command'].includes(target)) {
    ...
    const check = await this.diagnosticsService.runSetupTarget(target);
    this.writeJson(res, 200, {
      target,
      generatedAt: new Date().toISOString(),
      check,
      message: buildDiagnosticsSetupTestMessage(target, check),
      repairHint: buildDiagnosticsSetupRepairHint(target, check),
    });
  }
