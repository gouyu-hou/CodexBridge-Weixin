...
test('parseWeixinLoginArgs reads supported CLI flags', () => {
  ...
  assert.equal(parsed.saveFile, true);
  assert.equal(parseWeixinLoginArgs(['--no-save-file']).saveFile, false);
  assert.equal(parseWeixinLoginArgs([]).saveFile, null);
});
...
test('resolveQrArtifact can return the QR URL without saving a png', async () => {
  const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'codexbridge-cli-'));
  const result = await resolveQrArtifact({
    saveFile: false,
    stateDir: tmpDir,
    qrcode: 'qr-url-only',
    qrcodeImageContent: 'https://liteapp.weixin.qq.com/q/?qrcode=abc&bot_type=3',
  ...
  assert.equal(result.sourceUrl, 'https://liteapp.weixin.qq.com/q/?qrcode=abc&bot_type=3');
  assert.equal(fs.existsSync(path.join(tmpDir, 'weixin', 'login', 'qr-url-only.png')), false);
});
...
test('isServeLockOwnerCommandLine rejects unrelated PID reuse command lines', () => {
  assert.equal(
    isServeLockOwnerCommandLine('"D:\\download\\Tencent\\xwechat\\WeChatAppEx.exe" --type=gpu-process'),
    false,
  );
  assert.equal(
    isServeLockOwnerCommandLine('D:\\node.exe --import tsx C:\\repo\\CodexBridge\\src\\cli.ts weixin serve --state-dir C:\\state'),
    true,
  );
});
