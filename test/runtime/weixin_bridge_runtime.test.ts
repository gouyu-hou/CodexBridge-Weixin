...
test('WeixinBridgeRuntime sends only the final answer when progress previews are disabled', async () => {
  const sent: Array<{ externalScopeId: string; content: string }> = [];
  const runtime = makeRuntime({
    ...
    coordinator: {
      async handleInboundEvent(_event: any, options: any = {}) {
        await options.onProgress?.({
          text: 'I am checking the context first.',
          delta: 'I am checking the context first.',
        ...
        return completeResponse('Final answer only.');
      },
    },
  });
  ...
  assert.deepEqual(sent, [
    { externalScopeId: 'wxid_1', content: 'Final answer only.' },
  ]);
});

test('WeixinBridgeRuntime limits concurrent Codex turns across different scopes', async () => {
  let active = 0;
  let maxActive = 0;
  const seen: string[] = [];
  const sent: string[] = [];
  const runtime = makeRuntime({
    ...
    pollEvents: [
      { platform: 'weixin', externalScopeId: 'wxid_1', text: 'one' },
      { platform: 'weixin', externalScopeId: 'wxid_2', text: 'two' },
      { platform: 'weixin', externalScopeId: 'wxid_3', text: 'three' },
    ],
    ...
    coordinator: {
      async handleInboundEvent(event: any) {
        ...
        await sleep(25);
        active -= 1;
        return completeResponse(`answer ${event.text}`);
      },
    },
  });
  ...
  assert.deepEqual(new Set(sent), new Set(['answer one', 'answer two', 'answer three']));
  assert.equal(runtime.getStatus().maxConcurrentTurns, 2);
});

test('WeixinBridgeRuntime keeps the same scope serialized even when events are dispatched concurrently', async () => {
  let active = 0;
  let maxActive = 0;
  const seen: string[] = [];
  const sent: string[] = [];
  const runtime = makeRuntime({
    ...
    eventDispatchConcurrency: 8,
    pollEvents: [
      { platform: 'weixin', externalScopeId: 'wxid_same', text: 'first' },
      { platform: 'weixin', externalScopeId: 'wxid_same', text: 'second' },
    ],
    sendText: async ({ content }) => {
      sent.push(content);
    },
  ...
  assert.deepEqual(sent, ['answer first', 'answer second']);
});
...
test('WeixinBridgeRuntime sends typing stop immediately when /stop interrupts a busy scope', async () => {
  const typing: Array<{ externalScopeId: string; status: 'start' | 'stop' }> = [];
  ...
  const stopCountAfterStop = typing.filter((entry) => entry.status === 'stop').length;
  await new Promise((resolve) => setTimeout(resolve, 8));
  assert.equal(typing.filter((entry) => entry.status === 'stop').length, stopCountAfterStop);
...
test('WeixinBridgeRuntime keeps image-only inbound messages staged until text arrives', async () => {
  const seen: Array<{ text: string; attachmentCount: number }> = [];
  const sent: Array<{ externalScopeId: string; content: string }> = [];
  const runtime = makeRuntime({
    inboundAttachmentMergeWindowMs: 5,
    ...
    coordinator: {
      async handleInboundEvent(event: any) {
        ...
        return completeResponse('received staged image');
      },
    },
  });

  const stagedImage = runtime.dispatchInboundEvent({
    platform: 'weixin',
    externalScopeId: 'wxid_1',
    text: '',
    attachments: [{
  ...
  assert.deepEqual(sent, [
    { externalScopeId: 'wxid_1', content: 'received staged image' },
  ]);
});

test('WeixinBridgeRuntime stages multiple image-only messages and submits them with the next prompt', async () => {
  const seen: Array<{ text: string; attachmentPaths: string[] }> = [];
  const sent: Array<{ externalScopeId: string; content: string }> = [];
  const runtime = makeRuntime({
    inboundAttachmentMergeWindowMs: 5,
    previewSoftTargetBytes: 1024,
    ...
    coordinator: {
      async handleInboundEvent(event: any) {
        seen.push({
          text: event.text,
          attachmentPaths: Array.isArray(event.attachments)
            ? event.attachments.map((attachment: any) => attachment.localPath)
            : [],
        });
        return completeResponse('received both images');
      },
    },
  });

  const firstImage = runtime.dispatchInboundEvent({
    platform: 'weixin',
    externalScopeId: 'wxid_1',
    text: '',
    attachments: [{
      kind: 'image',
      localPath: '/tmp/codexbridge-image-a.png',
    }],
  });
  const secondImage = runtime.dispatchInboundEvent({
    platform: 'weixin',
    externalScopeId: 'wxid_1',
    text: '',
    attachments: [{
      kind: 'image',
      localPath: '/tmp/codexbridge-image-b.png',
    }],
  });
  await sleep(20);
  ...
  await runtime.dispatchInboundEvent({
    platform: 'weixin',
    externalScopeId: 'wxid_1',
    text: 'analyze these two images together',
  });
  await runtime.waitForIdle();
  await firstImage;
  await secondImage;
  ...
  assert.deepEqual(seen, [
    {
      text: 'analyze these two images together',
      attachmentPaths: [
        '/tmp/codexbridge-image-a.png',
        '/tmp/codexbridge-image-b.png',
      ],
    },
  ]);
  assert.deepEqual(sent, [
    { externalScopeId: 'wxid_1', content: 'received both images' },
  ]);
});

test('WeixinBridgeRuntime does not flush image-only staged messages while stopping', async () => {
  const seen: Array<{ text: string; attachmentCount: number }> = [];
  const runtime = makeRuntime({
    inboundAttachmentMergeWindowMs: 5,
    previewSoftTargetBytes: 1024,
  ...
  const stagedImage = runtime.dispatchInboundEvent({
    ...
    attachments: [{
      kind: 'image',
      localPath: '/tmp/codexbridge-image-only-stop.png',
    }],
  });
  await sleep(20);
...
test('WeixinBridgeRuntime retries queued final text on recovery commands', async () => {
  const sent: Array<{ externalScopeId: string; content: string }> = [];
  let sendAttempts = 0;
  const restartActions: string[] = [];
  const restartMessage = [
    createI18n().t('coordinator.restart.queued'),
    createI18n().t('coordinator.restart.continue'),
  ].join('\n');
  const runtime = makeRuntime({
    sendText: async ({ externalScopeId, content }) => {
      sendAttempts += 1;
    ...
    coordinator: {
      async restartBridge() {
        restartActions.push('restart');
      },
      async handleInboundEvent() {
        return completeResponse('queued final answer');
      },
    },
  });
  ...
  assert.deepEqual(sent, [
    { externalScopeId: 'wxid_1', content: 'queued final answer' },
    { externalScopeId: 'wxid_1', content: restartMessage },
  ]);
...
test('WeixinBridgeRuntime queues a second plain-text message as an appended follow-up during an active scope turn', async () => {
  const sent: Array<{ externalScopeId: string; content: string }> = [];
...
test('WeixinBridgeRuntime merges multiple messages appended while the scope is busy', async () => {
  const sent: Array<{ externalScopeId: string; content: string }> = [];
  const started: string[] = [];
  let releaseFirst: (value?: unknown) => void = () => {};
  const firstGate = new Promise((resolve) => {
  ...
  const runtime = makeRuntime({
    ...
    coordinator: {
      async handleInboundEvent(event: any) {
        ...
        return completeResponse(`merged: ${event.text}`);
      },
    },
  });
  ...
  assert.deepEqual(sent, [
    { externalScopeId: 'wxid_1', content: 'first answer' },
    { externalScopeId: 'wxid_1', content: 'merged: second\n\nthird' },
  ]);
});

test('WeixinBridgeRuntime ignores duplicate Weixin inbound message ids', async () => {
  const sent: Array<{ externalScopeId: string; content: string }> = [];
  const started: string[] = [];
  const runtime = makeRuntime({
    pollEvents: [
      {
        ...
        externalScopeId: 'wxid_1',
        text: 'same',
        metadata: { weixin: { messageId: 'msg-1', senderId: 'user-1' } },
      },
      {
        platform: 'weixin',
        externalScopeId: 'wxid_1',
  ...
  assert.deepEqual(sent, [
    { externalScopeId: 'wxid_1', content: 'answer: same' },
  ]);
});

test('WeixinBridgeRuntime suppresses complete commentary-only text instead of sending it as the final answer', async () => {
  const sent: Array<{ externalScopeId: string; content: string }> = [];
  const runtime = makeRuntime({
    sendText: async ({ externalScopeId, content }) => {
      sent.push({ externalScopeId, content });
    ...
    coordinator: {
      async handleInboundEvent() {
        return {
          type: 'message',
          messages: [{ text: 'I am thinking through the problem step by step.' }],
          meta: {
            codexTurn: {
              outputState: 'complete',
              previewText: '',
  ...
  assert.deepEqual(sent, [
    {
      externalScopeId: 'wxid_1',
      content: createI18n().t('runtime.error.finalAnswerMissing'),
    },
  ]);
});
...
test('WeixinBridgeRuntime does not send commentary-only partial text as a final answer', async () => {
  const sent = [];
  const runtime = makeRuntime({
    sendText: async ({ externalScopeId, content }) => {
      sent.push({ externalScopeId, content });
  ...
  assert.deepEqual(sent, [
    { externalScopeId: 'wxid_1', content: createI18n().t('runtime.error.finalAnswerMissing') },
  ]);
});
...
test('WeixinBridgeRuntime restart command clears a stuck scope and suppresses stale replies', async () => {
  const sent: Array<{ externalScopeId: string; content: string }> = [];
  const actions: string[] = [];
  let releaseSlowTurn: (() => void) | null = null;
  let slowTurnStarted: (() => void) | null = null;
  const slowTurnStartedPromise = new Promise<void>((resolve) => {
    slowTurnStarted = resolve;
  });
  const slowTurnGate = new Promise<void>((resolve) => {
  ...
  const runtime = makeRuntime({
    ...
    coordinator: {
      async forceClearActiveTurn() {
        actions.push('clear-active');
      },
      async restartBridge() {
        actions.push('restart');
      },
      async handleInboundEvent(event: any) {
        if (event.text === 'slow') {
          slowTurnStarted?.();
          await slowTurnGate;
          return completeResponse('stale slow answer');
        }
        return completeResponse('unused');
      },
    },
  ...
  const slowTurn = runtime.handleInboundEvent({
    platform: 'weixin',
    externalScopeId: 'wxid_1',
    text: 'slow',
  });
  await slowTurnStartedPromise;
...
function makeMetricsRuntime(stateDir: string) {
  ...
  const runtime: any = new WeixinBridgeRuntime({
    ...
    bridgeCoordinator: { async handleInboundEvent() { return { type: 'message', messages: [] }; } },
    metricsStore,
    locale: null,
  } as any);
  return { runtime, metricsStore };
}

test('WeixinBridgeRuntime keys per-account metrics and persists them across restart', async () => {
  const stateDir = fs.mkdtempSync(path.join(os.tmpdir(), 'cb-rt-metrics-'));
  const { runtime, metricsStore } = makeMetricsRuntime(stateDir);

  assert.equal(runtime.accountKeyOf({ metadata: { weixinAccountId: 'acctA' } }), 'acctA');
  assert.equal(runtime.accountKeyOf({}), 'default');
  ...
  assert.equal(metrics.byAccount.acctA.turnsCompleted, 1);
  assert.equal(metrics.byAccount.acctA.turnsFailed, 0);
...
test('postAlert debounces identical alerts and ignores non-http urls', async () => {
  ...
  try {
    assert.equal(await postAlert('', { type: 't', stage: 's', message: 'm', at: 1 }), false);
    assert.equal(await postAlert('not-a-url', { type: 't', stage: 's', message: 'm', at: 1 }), false);
    assert.equal(calls.length, 0);

    assert.equal(await postAlert('http://h/x', { type: 'weixin_error', stage: 'poll', message: 'boom', at: 1 }, { minIntervalMs: 60000 }), true);
    assert.equal(calls.length, 1);
    assert.equal(await postAlert('http://h/x', { type: 'weixin_error', stage: 'poll', message: 'boom', at: 2 }, { minIntervalMs: 60000 }), false);
    assert.equal(calls.length, 1);
    assert.equal(await postAlert('http://h/x', { type: 'weixin_error', stage: 'poll', message: 'other', at: 3 }, { minIntervalMs: 60000 }), true);
