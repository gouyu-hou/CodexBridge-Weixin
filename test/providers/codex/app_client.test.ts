...
test('CodexAppClient merges Codex Desktop session_index titles into thread lists and reads', async () => {
  const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), 'codexbridge-session-index-'));
  const sessionDir = path.join(tempDir, 'sessions');
  fs.mkdirSync(sessionDir, { recursive: true });
  fs.writeFileSync(path.join(sessionDir, 'desktop-thread-1.jsonl'), '{}\n', 'utf8');
  fs.writeFileSync(path.join(tempDir, 'session_index.jsonl'), [
    JSON.stringify({
      id: 'desktop-thread-1',
      thread_name: '接入微信聊天',
      updated_at: '2026-06-24T07:49:18.2836116Z',
      path: 'sessions/desktop-thread-1.jsonl',
    }),
    JSON.stringify({
      id: 'deleted-thread-1',
      thread_name: '接入旧标题残留',
      updated_at: '2026-06-25T08:49:18.2836116Z',
    }),
    JSON.stringify({
      id: 'server-thread-1',
      thread_name: '桌面补全标题',
      updated_at: '2026-06-25T07:49:18.2836116Z',
    }),
  ].join('\n'), 'utf8');
  const client = new CodexAppClient({
    codexCliBin: 'codex',
  ...
  client.request = async (method, params) => {
    if (method === 'thread/list') {
      assert.equal(params.searchTerm, '接入');
      return {
        data: [{
          id: 'server-thread-1',
          name: '',
  ...
  const listed = await client.listThreads({ searchTerm: '接入' });
  assert.equal(listed.items.some((item) => item.threadId === 'desktop-thread-1' && item.title === '接入微信聊天'), true);
  assert.equal(listed.items.some((item) => item.threadId === 'deleted-thread-1'), false);
  assert.equal(listed.items.some((item) => item.threadId === 'server-thread-1' && item.title === '桌面补全标题'), true);

  const read = await client.readThread('server-thread-1');
  assert.equal(read?.title, '桌面补全标题');
});

test('CodexAppClient falls back to session_index when thread/read cannot materialize a desktop thread', async () => {
  const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), 'codexbridge-session-index-read-'));
  const sessionDir = path.join(tempDir, 'sessions');
  fs.mkdirSync(sessionDir, { recursive: true });
  fs.writeFileSync(path.join(sessionDir, 'desktop-thread-2.jsonl'), '{}\n', 'utf8');
  fs.writeFileSync(path.join(tempDir, 'session_index.jsonl'), JSON.stringify({
    id: 'desktop-thread-2',
    thread_name: '分析世界杯两场比分',
    updated_at: '2026-06-26T05:00:11.6284404Z',
    path: 'sessions/desktop-thread-2.jsonl',
  }), 'utf8');
  const client = new CodexAppClient({
    codexCliBin: 'codex',
    codexHome: tempDir,
  ...
  client.request = async (method) => {
    ...
    throw new Error('thread/read failed: thread not found: desktop-thread-2');
  };

  const read = await client.readThread('desktop-thread-2');
  assert.equal(read?.threadId, 'desktop-thread-2');
  assert.equal(read?.title, '分析世界杯两场比分');
});

test('CodexAppClient ignores stale session_index entries without a session file', async () => {
  const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), 'codexbridge-session-index-stale-'));
  fs.writeFileSync(path.join(tempDir, 'session_index.jsonl'), JSON.stringify({
    id: 'deleted-thread-2',
    thread_name: '已经删除的桌面会话',
    updated_at: '2026-06-26T05:00:11.6284404Z',
  }), 'utf8');
  const client = new CodexAppClient({
    codexCliBin: 'codex',
  ...
  client.request = async (method) => {
    ...
    if (method === 'thread/read') {
      throw new Error('thread/read failed: thread not found: deleted-thread-2');
    }
    throw new Error(`Unexpected method: ${method}`);
  };

  const listed = await client.listThreads({ searchTerm: '已经删除' });
  assert.equal(listed.items.some((item) => item.threadId === 'deleted-thread-2'), false);

  const read = await client.readThread('deleted-thread-2');
...
test('CodexAppClient treats unsupported plugin list RPC as an empty plugin catalog', async () => {
  const client = new CodexAppClient({
    codexCliBin: 'codex',
  });
  ...
  client.request = async (method) => {
    assert.equal(method, 'plugin/list');
    throw new Error('Invalid request: unknown variant `plugin/list`, expected one of `initialize`, `thread/start`');
  };
...
test('CodexAppClient treats unsupported app list RPC as no apps', async () => {
  const client = new CodexAppClient({
    codexCliBin: 'codex',
  });
  ...
  client.request = async (method) => {
    assert.equal(method, 'app/list');
    throw new Error('Invalid request: unknown variant `app/list`, expected one of `initialize`, `thread/start`');
  };
...
test('CodexAppClient startServer falls back to legacy stdio listen argument', async () => {
  const calls = [];
  ...
  secondChild.exitCode = null;

  const client = new CodexAppClient({
    codexCliBin: 'codex',
    spawnImpl: ((command, args, options) => {
      calls.push({ command, args, options });
      return calls.length === 1 ? firstChild as any : secondChild as any;
    }) as any,
  });
...
test('CodexAppClient startServer falls back to websocket when stdio auto-start fails', async () => {
  const calls = [];
  const stdioChildOne = new EventEmitter() as EventEmitter & {
    stderr: EventEmitter;
    stdout: EventEmitter;
    exitCode: number | null;
  };
  stdioChildOne.stderr = new EventEmitter();
  stdioChildOne.stdout = new EventEmitter();
  stdioChildOne.exitCode = 1;
  const stdioChildTwo = new EventEmitter() as EventEmitter & {
    stderr: EventEmitter;
    stdout: EventEmitter;
    exitCode: number | null;
  };
  stdioChildTwo.stderr = new EventEmitter();
  stdioChildTwo.stdout = new EventEmitter();
  stdioChildTwo.exitCode = 1;
  ...
  const client = new CodexAppClient({
    ...
    spawnImpl: ((command, args, options) => {
      calls.push({ command, args, options });
      if (calls.length === 1) {
        return stdioChildOne as any;
      }
      if (calls.length === 2) {
        return stdioChildTwo as any;
      }
      return websocketChild as any;
    }) as any,
  });
  ...
  assert.deepEqual(calls[0]?.args, ['app-server']);
  assert.deepEqual(calls[1]?.args?.slice(1, 3), ['--listen', 'stdio://']);
  assert.deepEqual(calls[2]?.args?.slice(1, 2), ['--listen']);
  assert.match(String(calls[2]?.args?.[2]), /^ws:\/\/127\.0\.0\.1:\d+$/);
...
test('CodexAppClient startServer wraps Windows cmd launchers through cmd.exe', async () => {
  ...
  assert.match(String(calls[0]?.command), /^"C:\\Program Files\\Codex\\codex\.cmd" app-server$/);
});
