...
test('CodexProviderPlugin turns inbound attachments into text prompt plus localImage inputs', async () => {
  ...
  assert.match(seenInput?.[0]?.text ?? '', /Attached images?: 1/);
  assert.match(seenInput?.[0]?.text ?? '', /report\.pdf/);
  assert.doesNotMatch(seenInput?.[0]?.text ?? '', /example\.png/);
  assert.doesNotMatch(seenInput?.[0]?.text ?? '', /image\/png/);
  assert.deepEqual(seenInput?.[1], {
    type: 'localImage',
    path: '/tmp/example.png',
  });
  assert.match(String(seenInputText ?? ''), /Attached images?: 1/);
});
