...
test('bridge coordinator tells the user when an external app share cannot be read', async () => {
  ...
  const result = await runtime.services.bridgeCoordinator.handleInboundEvent({
    platform: 'weixin',
    externalScopeId: 'wx-user-unreadable-share-1',
    text: '7:/ 05/14 :5pm J@I.II 【06/25 世界杯6场进球数深度分析（基于情报+转化率）】长按复制打开抖音，即可阅读文章 ŠŠbejSqMm4x79^^\n帮我按照这个文章分析',
  });

  assert.equal(result.type, 'message');
  assert.match(result.messages[0]?.text ?? '', /无法直接读取原文内容/);
  assert.equal(openai.startThreadCalls.length, 0);
  assert.equal(openai.startTurnCalls.length, 0);
});

test('bridge coordinator lets external app shares with pasted article content proceed', async () => {
  const { runtime, openai } = makeRuntime();
  const articleBody = [
    '正文：日本近期前场压迫效率提升，边路传中次数稳定，瑞典中卫组合回追速度偏慢。',
    '两队近五场都有较高射门转化率，日本在领先后仍会保持反击速度，瑞典定位球威胁明显。',
    '如果上半场早早进球，比赛节奏会被拉开，后续大概率出现更多反击空间。',
    '综合防守漏洞、进攻转化率和赛程体能，本场更倾向于中高进球区间。',
  ].join('\n');

  const result = await runtime.services.bridgeCoordinator.handleInboundEvent({
    platform: 'weixin',
    externalScopeId: 'wx-user-readable-share-1',
    text: `7:/ 05/14 :5pm J@I.II 【06/25 世界杯6场进球数深度分析】长按复制打开抖音，即可阅读文章 ŠŠbejSqMm4x79^^\n${articleBody}\n帮我分析进球数概率`,
  });
...
test('bridge coordinator lets external app shares with attachments proceed', async () => {
  const { runtime, openai } = makeRuntime();
  const imagePath = createTempAttachment('share-screenshot.png', 'png');

  const result = await runtime.services.bridgeCoordinator.handleInboundEvent({
    platform: 'weixin',
    externalScopeId: 'wx-user-readable-share-image-1',
    text: '7:/ 05/14 :5pm J@I.II 【06/25 世界杯6场进球数深度分析】长按复制打开抖音，即可阅读文章 ŠŠbejSqMm4x79^^\n看这张截图分析',
    attachments: [{
      kind: 'image',
      localPath: imagePath,
      fileName: 'share-screenshot.png',
      mimeType: 'image/png',
    }],
  });
...
test('/as edit modifies the pending assistant record instead of replacing it', async () => {
  ...
  assert.equal(new Date(after?.remindAt ?? 0).getUTCHours(), 11);
...
test('/compact updates the current binding when native compaction returns a replacement thread id', async () => {
  ...
  await runtime.services.bridgeCoordinator.handleInboundEvent({
    ...
    text: 'create compact rebound thread',
  });
  const session = runtime.services.bridgeSessions.resolveScopeSession({
    platform: 'weixin',
    externalScopeId: 'wx-user-compact-2',
...
test('/compact returns a guided stale-thread message when the bound native thread no longer exists', async () => {
  ...
  await runtime.services.bridgeCoordinator.handleInboundEvent({
    ...
    text: 'create stale compact thread',
  });
  const session = runtime.services.bridgeSessions.resolveScopeSession({
    platform: 'weixin',
    externalScopeId: 'wx-user-compact-4',
...
test('/new waits for the next normal message before creating a fresh session', async () => {
  const { runtime, openai } = makeRuntime();
  await runtime.services.bridgeCoordinator.handleInboundEvent({
    platform: 'weixin',
    externalScopeId: 'wx-user-1',
  ...
  const stillCurrent = runtime.services.bridgeSessions.resolveScopeSession({
    platform: 'weixin',
    externalScopeId: 'wx-user-1',
  });

  assert.match(result.messages[0]?.text ?? '', /已准备新会话/);
  assert.equal(openai.startThreadCalls.length, 1);
  ...
  assert.match(commandResult.messages.map((message) => message.text ?? '').join('\n'), /等待下一条普通内容后创建/);
  assert.equal(openai.startThreadCalls.length, 1);
  ...
  const lastRetryableRequest = runtime.services.bridgeSessions.getSessionSettings(rebound.id)
    ?.metadata?.lastRetryableRequest as Record<string, unknown> | null | undefined;
  assert.equal(lastRetryableRequest?.text, 'first real message');
});

test('/new pending request is cancelled when opening another thread before sending content', async () => {
  const { runtime, openai } = makeRuntime();
  await runtime.services.bridgeCoordinator.handleInboundEvent({
    platform: 'weixin',
    externalScopeId: 'wx-user-new-open-1',
    text: 'hello original',
  });
  const openedThread = await openai.startThread({
    providerProfile: makeProviderProfile('openai-default', 'openai-native', 'OpenAI Default'),
    cwd: '/tmp/opened-after-new',
    title: 'Opened after pending new',
    metadata: {},
  });
  ...
  await runtime.services.bridgeCoordinator.handleInboundEvent({
    platform: 'weixin',
    externalScopeId: 'wx-user-new-open-1',
    text: `/open ${openedThread.threadId}`,
  });
  const afterOpen = runtime.services.bridgeSessions.resolveScopeSession({
    platform: 'weixin',
    externalScopeId: 'wx-user-new-open-1',
  });
  assert.equal(afterOpen?.codexThreadId, openedThread.threadId);

  await runtime.services.bridgeCoordinator.handleInboundEvent({
    platform: 'weixin',
    externalScopeId: 'wx-user-new-open-1',
    text: 'message after open',
  });

  assert.equal(openai.startThreadCalls.length, 2);
  assert.equal(openai.startTurnCalls.at(-1)?.bridgeSession?.codexThreadId, openedThread.threadId);
});

test('/new pending request accepts settings commands before the first normal message', async () => {
  const { runtime, openai } = makeRuntime();
  const scope = {
    platform: 'weixin' as const,
    externalScopeId: 'wx-user-pending-settings-1',
  };
  ...
  const pendingStatus = await runtime.services.bridgeCoordinator.handleInboundEvent({
    ...scope,
    text: '/status',
  });
  assert.match(
    pendingStatus.messages.map((message) => message.text ?? '').join('\n'),
    /等待下一条普通内容后创建/,
  );
  ...
  assert.ok(rebound);
  const settings = runtime.services.bridgeSessions.getSessionSettings(rebound.id);
  assert.equal(settings?.personality, 'pragmatic');
...
test('/new pending request can switch provider and locale before the first normal message', async () => {
  const { runtime } = makeRuntime();
  const scope = {
    platform: 'weixin' as const,
    externalScopeId: 'wx-user-pending-provider-1',
  };
  ...
  const rebound = runtime.services.bridgeSessions.resolveScopeSession(scope);
  assert.ok(rebound);
  assert.equal(rebound.providerProfileId, 'compat-default');
  const settings = runtime.services.bridgeSessions.getSessionSettings(rebound.id);
...
test('/threads renders a paged thread browser with previews and commands', async () => {
  ...
  assert.match(text, /------\n\n常用命令：/);
  assert.match(text, /\/new        新会话/);
  assert.match(text, /\/up         开启上传模式，连续上传文件\/图片后再统一提交/);
  assert.match(text, /\/rename this name    给当前会话改名为“name”/);
  assert.match(text, /\/compact    压缩上下文/);
});

test('/threads preview prefers the latest local user prompt over provider preview', async () => {
  const { runtime, openai } = makeRuntime();

  await runtime.services.bridgeCoordinator.handleInboundEvent({
    platform: 'weixin',
    externalScopeId: 'wx-user-latest-preview',
    text: 'first question from wx',
  });
  const second = await runtime.services.bridgeCoordinator.handleInboundEvent({
    platform: 'weixin',
    externalScopeId: 'wx-user-latest-preview',
    text: 'second latest question from wx',
  });
  const thread = openai.threads.get(second.session?.codexThreadId);
  openai.threads.set(second.session?.codexThreadId, {
    ...thread,
    preview: 'first question from wx',
  });
  ...
  const listText = listResult.messages[0]?.text ?? '';
  assert.match(listText, /second latest question from wx/);
  assert.doesNotMatch(listText, /预览：first question from wx/);

  const searchResult = await runtime.services.bridgeCoordinator.handleInboundEvent({
    platform: 'weixin',
    externalScopeId: 'wx-user-latest-preview',
    text: '/search second latest',
  });
  assert.match(searchResult.messages[0]?.text ?? '', /second latest question from wx/);
});
...
test('/next and /prev paginate the current thread browser page', async () => {
  ...
  assert.match(firstPage.messages[0]?.text ?? '', /新线程00021/);
...
test('/threads keeps recently renamed local titles visible ahead of provider default titles', async () => {
  const { runtime, openai } = makeRuntime();

  const renamedThread = await runtime.services.bridgeCoordinator.handleInboundEvent({
    platform: 'weixin',
    externalScopeId: 'wx-renamed-title-visible',
    text: 'renamed local title target',
  });

  const providerProfile = makeProviderProfile('openai-default', 'openai-native', 'OpenAI Default');
  for (let index = 0; index < 5; index += 1) {
    await openai.startThread({
      providerProfile,
      cwd: '/tmp/codexbridge-title-visible',
      title: `新线程0000${index + 1}`,
      metadata: {},
    });
  }
  await runtime.services.bridgeCoordinator.handleInboundEvent({
  ...
  const text = result.messages[0]?.text ?? '';
  assert.match(text, /手机买球分析/);
  assert.doesNotMatch(text, new RegExp(`\\*\\s+\\d+\\.\\s+${renamedThread.session?.title ?? '新线程00001'}`));

  const opened = await runtime.services.bridgeCoordinator.handleInboundEvent({
    platform: 'weixin',
    externalScopeId: 'wx-renamed-title-browser',
    text: '/open 手机买球分析',
  });
  assert.equal(opened.session?.codexThreadId, renamedThread.session?.codexThreadId);
});

test('/rename current updates the bound thread without needing a list index', async () => {
  const { runtime } = makeRuntime();

  const created = await runtime.services.bridgeCoordinator.handleInboundEvent({
    platform: 'weixin',
    externalScopeId: 'wx-rename-current-1',
    text: 'current rename candidate',
  });

  const renamed = await runtime.services.bridgeCoordinator.handleInboundEvent({
    platform: 'weixin',
    externalScopeId: 'wx-rename-current-1',
    text: '/rename 当前 Current Soccer Analysis',
  });
  assert.match(renamed.messages[0]?.text ?? '', /Current Soccer Analysis/);
  assert.match(renamed.messages[0]?.text ?? '', new RegExp(String(created.session?.codexThreadId)));
  ...
  assert.ok(status.messages.some((message) => /Current Soccer Analysis/.test(String(message.text ?? ''))));
});

test('/rename rejects duplicate local thread names and /open accepts a unique name', async () => {
  const { runtime } = makeRuntime();

  const first = await runtime.services.bridgeCoordinator.handleInboundEvent({
    platform: 'weixin',
    externalScopeId: 'wx-named-thread-1',
    text: 'first named thread',
  });
  const second = await runtime.services.bridgeCoordinator.handleInboundEvent({
    platform: 'weixin',
    externalScopeId: 'wx-named-thread-2',
    text: 'second named thread',
  });

  await runtime.services.bridgeCoordinator.handleInboundEvent({
    platform: 'weixin',
    externalScopeId: 'wx-name-browser',
    text: '/threads',
  });
  const renamed = await runtime.services.bridgeCoordinator.handleInboundEvent({
    platform: 'weixin',
    externalScopeId: 'wx-name-browser',
    text: '/rename 2 Project Alpha',
  });
  assert.match(renamed.messages[0]?.text ?? '', /Project Alpha/);

  const duplicate = await runtime.services.bridgeCoordinator.handleInboundEvent({
    platform: 'weixin',
    externalScopeId: 'wx-name-browser',
    text: '/rename 1 project   alpha',
  });
  assert.match(duplicate.messages[0]?.text ?? '', /同名会话|already exists/);
  ...
  const openedByNormalizedName = await runtime.services.bridgeCoordinator.handleInboundEvent({
    platform: 'weixin',
    externalScopeId: 'wx-name-browser',
    text: '/open project   alpha',
  });
  assert.equal(openedByNormalizedName.session?.codexThreadId, first.session?.codexThreadId);

  const unchangedSecondThread = await runtime.services.bridgeCoordinator.handleInboundEvent({
    platform: 'weixin',
    externalScopeId: 'wx-name-browser',
    text: `/open ${second.session?.codexThreadId}`,
  });
  assert.equal(unchangedSecondThread.session?.codexThreadId, second.session?.codexThreadId);
});

test('/search matches local thread aliases after rename', async () => {
  const { runtime } = makeRuntime();

  await runtime.services.bridgeCoordinator.handleInboundEvent({
    platform: 'weixin',
    externalScopeId: 'wx-alias-search-1',
    text: 'plain underlying provider title',
  });

  await runtime.services.bridgeCoordinator.handleInboundEvent({
    platform: 'weixin',
    externalScopeId: 'wx-alias-search-browser',
    text: '/threads',
  });
  await runtime.services.bridgeCoordinator.handleInboundEvent({
    platform: 'weixin',
    externalScopeId: 'wx-alias-search-browser',
    text: '/rename 1 Local Alias Search Target',
  });
  const result = await runtime.services.bridgeCoordinator.handleInboundEvent({
    platform: 'weixin',
    externalScopeId: 'wx-alias-search-browser',
    text: '/search local alias',
  });

  const text = result.messages[0]?.text ?? '';
  assert.match(text, /Local Alias Search Target/);
  assert.doesNotMatch(text, /没有找到匹配的线程|No matching threads/);
});
...
test('/threads del archives a thread from the default list, and /threads restore revives it from the all view', async () => {
  ...
  assert.match(allView.messages[0]?.text ?? '', /常用命令：/);
  assert.match(allView.messages[0]?.text ?? '', /\/threads  看历史会话列表/);
...
test('/threads pin and /threads unpin keep pinned threads at the top and support pinned-only view', async () => {
  ...
  assert.match(pinnedViewText, /常用命令：/);
  assert.match(pinnedViewText, /\/threads  看历史会话列表/);
