...
const symlinkAwareTest = supportsSymlinkCreation() ? test : test.skip;

symlinkAwareTest('finalizeTurnArtifacts rejects symlinked manifest files that escape the turn artifact directory', () => {
...
function supportsSymlinkCreation(): boolean {
  const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), 'codexbridge-symlink-probe-'));
  try {
