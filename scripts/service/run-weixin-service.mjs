...
function writeChunk(streamName, chunk) {
  ...
  if (logPath) {
    fs.appendFile(logPath, text, (error) => {
      ...
      // Surface disk-full / permission failures instead of silently dropping logs.
      // Throttle so a persistent failure does not flood stderr.
      const now = Date.now();
      if (now - lastLogAppendErrorAt > 5000) {
        lastLogAppendErrorAt = now;
        process.stderr.write(`[codexbridge-service] log append failed (${logPath}): ${error.message}\n`);
      }
    });
  }
}
...
function defaultServiceEnvFile() {
  return path.join(rootDir, 'weixin.service.env');
}
